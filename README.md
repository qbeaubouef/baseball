...
| `public/index.html` | Entire frontend in one file. PIN gate, tab navigation, scoreboard with inline game expansion, standings tables, playoff picture. Fetches team colors from `/api/colors`. |
...
| `mlb-install.sh` | Proxmox VE helper script (tteck-style). Creates LXC, installs Node 20, deploys app + CSV, creates systemd service. |
